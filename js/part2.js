...
async function callAiForXiaobaixTask(index, button) {
    const task = xiaobaixTasksData[index];
    if (!task) return;

    getAiGuidance('描述你想要的变量初始化逻辑', async userGuidance => {
        if (!userGuidance) {
            alert('请输入你想要实现的变量初始化描述');
            return;
        }
        ...
        try {
            const cardData = buildCardObject ? buildCardObject() : {};
            ...
            const prompt = getLanguagePrefix() + `你是一个SillyTavern小白X插件的任务脚本编写专家。请根据角色设定和用户需求，生成小白X循环任务的 <<taskjs>> 代码。

**角色信息:**
- 角色名: ${characterContext.name || '未指定'}
- 描述: ${characterContext.description || '未指定'}
- 个性: ${characterContext.personality || '未指定'}
- 场景: ${characterContext.scenario || '未指定'}

**用户需求:** ${userGuidance}

**STscript 可用命令参考:**
- /setvar key=变量名 值 - 设置本地变量
- /getvar key=变量名 - 获取本地变量
- /addvar key=变量名 值 - 给变量追加/累加
- /setglobalvar key=变量名 值 - 设置全局变量
- /getglobalvar key=变量名 - 获取全局变量
- /incvar 变量名 - 变量加1
- /decvar 变量名 - 变量减1
- /flushvar 变量名 - 删除变量
- /listvar - 列出所有变量
- /echo 内容 - 输出内容
- 支持的数据类型: STRING, NUMBER, BOOLEAN, LIST [], DICTIONARY {}

**变量管理2.0 (LWB_StateV2):**
- await LWB_StateV2.applyText(\`<state>\n$schema 数据\n  字段: ""\n数据:\n  字段: "值"\n</state>\`, { silent: true });

**要求:**
1. 代码必须包裹在 <<taskjs>>...<</taskjs>> 中
2. 使用 await STscript() 调用酒馆命令
3. 如果需要变量管理2.0系统，使用 await LWB_StateV2.applyText()
4. 返回纯净的代码，不要Markdown代码块标记，不要额外解释`;
            ...
            if (result) {
                ...
                if (!code.startsWith('<<taskjs>>')) {
                    code = '<<taskjs>>\n' + code + '\n<</taskjs>>';
                }
                updateXiaobaixTask(index, 'commands', code);
                renderXiaobaixTasks();
                setTimeout(() => {
                    const container = document.getElementById('xiaobaix-tasks-container');
                    const card = container.querySelector('.xiaobaix-task-card[data-task-index="' + index + '"]');
                    if (card) card.classList.remove('collapsed');
                }, 100);
            }
        } catch (err) {
            console.error('AI生成小白X任务失败:', err);
